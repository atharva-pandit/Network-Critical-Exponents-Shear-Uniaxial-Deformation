...
int main( int argc, char* argv[] )
{
	...
	/* DEFINE Z & EXTENSION USING TERMINAL INPUT ---------------------------------------- */
	...
	readPoint ( pntfile, points, W, Z, V );
	readBond ( bndfile, bonds, W, Z, V );
	...
	while ( (std::abs(deform) - std::abs(def)) > 1e-6 )
	{
		...
		energy = FIRE_spring ( points, bonds, timestep, W, shiftB, strain, def );
	}
 	...
 	for (int tstep=0; tstep<steps; tstep++)
	{
		...
		energy = FIRE_spring ( points, bonds, timestep, W, shiftB, strain, deform );
		writeEnergy<double>( strain, energy, enefile );
